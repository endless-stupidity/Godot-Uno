...
func clear_cpu1_hand() -> void: #kiils all the children of the player hand
	for child in get_children():
		remove_child(child)

func update_cpu1_hand() -> void: #updates the player hand based on the GameMaster.player_hand
	var tween = create_tween().set_parallel()
	
	clear_cpu1_hand()
	var card_count = GameMaster.cpu1_hand.size() #how many cards are there
	...
	if card_count > 1:
		...
		var card = GameMaster.cpu1_hand[0]
		var hand_ratio = 0.5
